...
function Module:Ding(settings)
	...
	if settings[2] == "WHISPER" then
		...
		SendChatMessage(db.messages[random(1, #db.messages)]:gsub("#name#", sender):gsub("#level#", UnitLevel(sender)), settings[2], nil, settings[1])
	else
		SendChatMessage(db.messages[random(1, #db.messages)]:gsub("#name#", sender):gsub("#level#", UnitLevel(sender)), settings[2], nil)
	end
end
