...
function Module:ColorNames(frame)
	...
	elseif frame.petOwner == 2 and petName then
		if C_PetBattles.IsWildBattle(frame.petOwner, frame.PetIndex) then
			...
			if not highest then
				frame.Name:SetText(petString:format(hex, petName, "ffccff00", "Not Owned"))
			else
...
function Module:PET_BATTLE_OPENING_DONE()
	...
	if db.quality.enable and C_PetBattles.IsWildBattle(2, i) then
		local lineString = "|cffffff00Pet #%d:|r %s|Hbattlepet:%s:%s:%s:%s:%s:%s:|h[%s]|h|r |cffffffff(%d), Yours:|r %s"
		...
		for i = 1, C_PetBattles.GetNumPets(2) do
			...
			chatFrame:AddMessage(lineString:format(i, ITEM_QUALITY_COLORS[quality - 1].hex, species, level, quality, health, power, speed, C_PetBattles.GetName(2, i), level, extra))
		end
	end
end
