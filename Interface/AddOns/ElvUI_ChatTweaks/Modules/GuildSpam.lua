...
-- Based on BadBoy_Guilded by funkydude
local Module = ElvUI_ChatTweaks:NewModule("Guild Spam", "AceConsole-3.0")
local L = LibStub("AceLocale-3.0"):GetLocale("ElvUI_ChatTweaks", false)
Module.name = L["Guild Spam"]
Module.namespace = string.gsub(Module.name, " ", "")

local savedID, result = 0, nil
local players = {}
local notifyString = L["Guild Spam blocked a message from %s%s|r."]
	...
	for i = 1, #Module.triggers do
		if message:lower():find(Module.triggers[i]) then
			...
			if db.logging and Module.log:IsEnabled() then
				Module.log:AddToLog("guild", event, player, message)
			end
...
local function WhisperFilter(_, event, message, player)
	...
	for i = 1, #Module.whispers do
		if message:lower():find(Module.whispers[i]) then
			if db.notify then Module:Print(notifyString:format(L["Whisper"], player)) end
...
local function WhisperInformFilter(_, _, _, player)
	-- for tracking whispers
	if db.whispers and not players[player] then players[player] = true end
end
...
function Module:Info()
	return L["Filters guild recruitment spam in whispers or chat.  Can also block guild invites."]
end
	...
	if not options then
		options = {
			chat = {
				...
				order = 13,
				name = L["Chat"],
				desc = L["Filter guild recruitment messages in chat."],
				get = function() return db.chat end,
				set = function(_, value) db.chat = value end,
			},
			whispers = {
				type = "toggle",
				order = 14,
				name = L["Whispers"],
				desc = L["Filter guild recruitment whispers."],
				get = function() return db.whispers end,
				set = function(_, value) db.whispers = value end,
			},
			invites = {
				type = "toggle",
				order = 15,
			...
			notify = {
				...
				desc = L["Notify you when a message/whisper is filtered."],
