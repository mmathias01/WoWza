...
function MT:CreateBindingFrame()
	...
	frame:SetScript("OnShow",
		function()
			...
			local commandName = (sel > 100) and format("CLICK MacroToolkitSecureButton%d:LeftButton", sel) or format("MACRO %s", name)
...
function MT:UnbindKey(keyPressed)
	...
	if oldAction ~= "" and oldAction ~= MTB.selected then
		...
		if (not key1 or key1 == keyPressed) and (not key2 or key2 == keyPressed) then
			...
			local ubtext
			if string.find(bname, "CLICK MacroToolkitSecureButton") then
				local bnum = select(3, string.find(bname, "CLICK MacroToolkitSecureButton(%d+)"))
...
function MT:UpdateBindingFrame()
	...
	local commandName = (MacroToolkitFrame.selectedMacro > 100) and format("CLICK MacroToolkitSecureButton%d:LeftButton", MacroToolkitFrame.selectedMacro) or format("MACRO %s", name)
