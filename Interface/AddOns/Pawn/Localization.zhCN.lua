...
想查看更多关于Pawn的信息，请查看Pawn插件文件夹里的(Readme.htm) 文件。]=],
	...
	Stats = {
		...
		SpeedBaseline = "速度基线",
		SpeedBaselineInfo = "不是实际属性，该数字在乘以均衡标准前要减去速度属性。\", \"|cffffffff速度基线|r 是",
		...
		SpellPower = "法术强度",
		SpellPowerInfo = "法术强度.  出现在部分施法者武器/装备中.  不包括从智力获得的法术强度。",
		SpiritInfo = "主要属性，精神。",
		StaminaInfo = "主要属性，耐力。",
...
插件会根据你当前选择的角色，默认启用适合你的职业的评分尺度，或者你可以自己设定评分尺度。评分尺度的作用是根据给有用的每一点属性设定一个分数，以装备的总分去比较哪件装备更好更适合你的职业与天赋。评分会在装备的下方显示“评分数值与提升比例”，你可以在设置标签进行设置。

Pawn 的评分尺度默认是由Wowhead提供的标准适合大众的，你也可以禁用默认启用你自己的评分标准并进行调整，评分可以由网站导入、也可以导出分享给他人。
