...
For more information on customizing Pawn, please see the help file (Readme.htm) that comes with the mod.
	...
	Stats = {
		AgilityInfo = "Первичный параметр, Ловкость.",
		Ap = "Сила Атаки",
		...
		Sockets = "Гнезда",
		SpecialWeaponStats = "Спец статы оружия",
		SpeedBaseline = "Базовая скорость",
		SpeedBaselineInfo = "Число отнимается от Скорости оружия перед умножением на значение веса.",
		...
		SpellPowerInfo = "Сила заклинаний.  Присутствует на кастерском оружии, но не на броне.  Не включает силу заклинаний получаемую от инеллекта.",
	...
	TooltipParsing = {
		...
		WeaponDamageArcane = "^%+?([%d%.,]+) %- ([%d%.,]+) от тайной магии$",
		WeaponDamageEnchantment = "^%+?([%d%.,]+) к урону оружием$",
		WeaponDamageEquip = "^%+?([%d%.,]+) к урону от оружия%.$",
		WeaponDamageExact = "^Урон: %+?([%d%.,]+)$",
		WeaponDamageFire = "^%+?([%d%.,]+) %- ([%d%.,]+) от магии огня$",
		WeaponDamageFrost = "^%+?([%d%.,]+) %- ([%d%.,]+) от магии льда$",
		WeaponDamageHoly = "^%+?([%d%.,]+) %- ([%d%.,]+) от магии света$",
		WeaponDamageNature = "^%+?([%d%.,]+) %- ([%d%.,]+) от магии природы$",
		WeaponDamageShadow = "^%+?([%d%.,]+) %- ([%d%.,]+) от магии тьмы$",
	},
	UI = {
		AboutHeader = "О Pawn",
