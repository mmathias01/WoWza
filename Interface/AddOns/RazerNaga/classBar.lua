--[[
	classBar.lua
		Defines a RazerNaga class bar, which contains things like forms for druids, stances for warriors, auras for paladins, etc
--]]
...
local ClassButton = RazerNaga:CreateClass('CheckButton', RazerNaga.BindableButton)
...
function ClassButton:Create(id)
	local name = format('RazerNagaClassButton%d', id)
	...
	if not RazerNaga:Masque('Class Bar', b) then
...
local ClassBar = RazerNaga:CreateClass('Frame', RazerNaga.Frame)
RazerNaga.ClassBar  = ClassBar


function ClassBar:New()
	local f = self.super.New(self, 'class')
	f:SetTooltipText(L['ClassBarHelp_' .. select(2, UnitClass('player'))])
