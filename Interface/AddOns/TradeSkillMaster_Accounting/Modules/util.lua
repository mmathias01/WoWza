...
local function EncodeRecord(record, dType)
	...
	return encode(record.stackSize).."#"..encode(record.quantity).."#"..encode(floor(record.time)).."#"..encode(record.price).."#"..otherPerson.."#"..(record.player or "".."#"..record.source)
end

local function DecodeRecord(record, dType)
	local stackSize, quantity, sTime, price, otherPerson, player, source = ("#"):split(record)
...
function Util:DecodeItemData(dataType, itemData)
	...
	if dataType == "sold" and TSM.db.factionrealm.itemData.sell[itemData] then
		itemData = TSM.db.factionrealm.itemData.sell[itemData]
	elseif dataType == "buy" and TSM.db.factionrealm.itemData.buy[itemData] then
		itemData = TSM.db.factionrealm.itemData.buy[itemData]
	end
