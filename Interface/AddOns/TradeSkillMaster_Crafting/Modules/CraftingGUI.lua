...
function GUI:CreateCraftInfoFrame(parent)
	...
	for i = 1, MAX_TRADE_SKILL_REAGENTS do
		...
		if i == 1 then
			...
			btn:SetPoint("TOPLEFT", reagentButtons[i - 1], "BOTTOMLEFT", 0, -2)
			btn:SetPoint("TOPRIGHT", reagentButtons[i - 1], "BOTTOMRIGHT", 0, -2)
		end

		local quantityText = btn:CreateFontString()
		quantityText:SetFont(TSMAPI.Design:GetContentFont(), 13)
		quantityText:SetJustifyH("LEFT")
		quantityText:SetPoint("TOPLEFT", btn:GetFontString(), "TOPRIGHT")
		quantityText:SetPoint("BOTTOMRIGHT", btn)
		btn.quantityText = quantityText
						...
						for itemString, quantity in pairs(task.items) do
							...
							if task.taskType == L["Search for Mats"] or task.taskType == L["Visit Vendor"] or task.taskType == L["Collect Mail"] or task.taskType == L["Mail Items"] then
								row = {
									cols = {
										{
											value = format("    %s", color .. name .. " x" .. needQty)
										}
									},
								}
							else
