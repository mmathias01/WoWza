...
function Cancel:GetScanListAndSetup(GUIRef, options)
	...
	for i=GetNumAuctionItems("owner"), 1, -1 do
		...
		local apiIndex = 14
		if select(4, GetBuildInfo()) == 50400 then apiIndex = 16 end
		if select(apiIndex, GetAuctionItemInfo("owner", i)) == 0 then
...
function Cancel:ProcessItem(itemString, noLog)
	...
	for _, operation in pairs(operations) do
		...
		for i=GetNumAuctionItems("owner"), 1, -1 do
			local apiIndex = 14
			if select(4, GetBuildInfo()) == 50400 then apiIndex = 16 end
			if select(apiIndex, GetAuctionItemInfo("owner", i)) == 0 and itemString == TSMAPI:GetBaseItemString(GetAuctionItemLink("owner", i), true) then
...
function Cancel:SpecialScanProcessItem(itemString, noLog)
	...
	for i=GetNumAuctionItems("owner"), 1, -1 do
		...
		if select(4, GetBuildInfo()) == 50400 then apiIndex = 16 end
		if select(apiIndex, GetAuctionItemInfo("owner", i)) == 0 and itemString == TSMAPI:GetBaseItemString(GetAuctionItemLink("owner", i), true) then
			local _, _, quantity, _, _, _, _, bid, _, buyout, activeBid, _, _, wasSold, _, wasSold_54 = GetAuctionItemInfo("owner", i)
...
function Cancel:ShouldCancel(index, operation)
	local _, _, quantity, _, _, _, _, bid, _, buyout, activeBid, _, _, wasSold, _, wasSold_54 = GetAuctionItemInfo("owner", index)
	if select(4, GetBuildInfo()) == 50400 then wasSold = wasSold_54 end
